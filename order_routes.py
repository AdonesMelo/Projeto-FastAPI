...
async def pedidos():
    '''
    Essa é a rota padrão de pedidos do sistema. Todas rotas de pedidos precisam de autenticação!
    '''
...
async def criar_pedido(pedido_schema: PedidoSchema, session: Session = Depends(pegar_sessao)):
    ...
    session.commit()
    return {"Menssagem": f"Pedido criado com sucesso. ID do pedido: {novo_pedido.id}"}
...
async def cancelar_pedido(id_pedido: int, session: Session = Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    ...
    if not usuario.admin and usuario.id != pedido.usuario:
        raise HTTPException(status_code=401, detail="Você não tem autorização para modificar esse pedido")
    pedido.status = "CANCELADO"
    session.commit()
    return {
        "Menssagem": f"Pedido número: {pedido.id} cancelado com sucesso",
        "Pedido": pedido
    }
...
async def adcionar_item_pedido(id_pedido: int, 
    ...
    session.commit()
    return{
        "menssagem": "Item criado com sucesso",
...
async def remover_item_pedido(id_item_pedido: int, 
    ...
    session.commit()
    return{
        "menssagem": "Item removido com sucesso",
...
async def finalizar_pedido(id_pedido: int, session: Session = Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    ...
    session.commit()
    return {
        "Menssagem": f"Pedido número: {pedido.id} finalizado com sucesso",
        "Pedido": pedido
    }
